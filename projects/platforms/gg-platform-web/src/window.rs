...
impl WebWindow {
    ...
    pub fn new(config: WindowConfig) -> Self {
        Self {
            ...
            dom_event_queue: Arc::new(Mutex::new(Vec::new())),
        }
    }
            ...
            if let Some(window) = web_sys::window() {
                ...
                let resize_closure = wasm_bindgen::closure::Closure::wrap(Box::new(move || {
                    if let Some(w) = web_sys::window() {
                        let width =
                            w.inner_width().ok().and_then(|v| v.as_f64()).unwrap_or(0.0) as u32;
                        let height =
                            w.inner_height().ok().and_then(|v| v.as_f64()).unwrap_or(0.0) as u32;

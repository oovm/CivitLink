...
impl HtmlTextureCache {
    /// 创建新的纹理缓存
    pub fn new() -> Self {
        Self { raw_textures: HashMap::new(), piet_images: HashMap::new(), path_to_id: HashMap::new(), next_id: 1 }
    }
    ...
    pub fn reload_texture(&mut self, path: &str) -> GResult<()> {
        ...
        let data = std::fs::read(path_ref)
            .map_err(|e| GError { kind: GErrorKind::Io, message: format!("无法读取纹理文件 {:?}: {}", path_ref, e) })?;

        let img = image::load_from_memory(&data)
            .map_err(|e| GError {
                kind: GErrorKind::Asset, message: format!("无法解码纹理图像 {:?}: {}", path_ref, e)
            })?;
    ...
    pub fn get_or_create(&mut self, id: TextureId, rc: &mut impl PietRenderContext<Image = PietImage>) -> Option<&PietImage> {
        ...
        let piet_image = rc.make_image(raw.width, raw.height, &raw.data, piet_common::ImageFormat::RgbaSeparate).ok()?;
    ...
    /// 移除指定标识符的纹理
    ///
    /// 从缓存中移除原始纹理数据和关联的 piet 图像。
    /// 同时移除路径到标识符的映射中指向该标识符的条目。
    ///
    /// # 参数
    ///
    /// - `id` - 纹理标识符
    ...
    pub fn remove(&mut self, id: TextureId) -> bool {
        ...
        self.raw_textures.remove(&id).is_some()
    }
}

impl Default for HtmlTextureCache {
    fn default() -> Self {
        Self::new()
    }
}

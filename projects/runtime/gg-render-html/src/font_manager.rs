...
impl FontManager {
    ...
    pub fn new() -> Self {
        Self { font_family: None, font_data: None }
    }

    /// 从字节数据加载字体
    ///
    /// 将字体原始字节数据保存到管理器中，并尝试解析字体族名称。
    /// 使用 ttf-parser 解析字体族名称，供 piet 的 FontFamily 使用。
    ///
    /// # 参数
    ///
    /// - `data` - 字体文件的原始字节数据（如 TTF、OTF 格式）
    pub fn load_font(&mut self, data: Vec<u8>) -> GResult<()> {
        let family_name = ttf_parser::Face::parse(&data, 0)
            .map_err(|e| GError { kind: GErrorKind::Asset, message: format!("无法解析字体数据: {:?}", e) })?
            .names()
            .into_iter()
    ...
    pub fn load_font_from_path(&mut self, path: &Path) -> GResult<()> {
        let data = std::fs::read(path)
            .map_err(|e| GError {
                kind: GErrorKind::Io, message: format!("无法读取字体文件 '{}': {}", path.display(), e)
            })?;

use gg_core::GResult;
use gg_ecs::System;
use gg_ui::{EventSystem, UiEvent};

use super::{EventSystemResource, FocusManager, UiTreeResource};
    ...
    fn execute(&mut self, world: &mut gg_ecs::World) -> GResult<()> {
            ...
            if let Some(event_system) = world.get_resource_mut::<EventSystemResource>() {
                if let Some(ref tree) = tree {
                    if input.mouse_just_pressed {
                        let event = UiEvent::MouseDown { x: input.mouse_x, y: input.mouse_y };
                        if let Some(target_id) = EventSystem::hit_test(tree, input.mouse_x, input.mouse_y) {
                            event_system.0.dispatch_with_phases(&event, tree, target_id);
                        }
                    }
                    if input.mouse_just_released {
                        let up_event = UiEvent::MouseUp { x: input.mouse_x, y: input.mouse_y };
                        if let Some(target_id) = EventSystem::hit_test(tree, input.mouse_x, input.mouse_y) {
                            event_system.0.dispatch_with_phases(&up_event, tree, target_id);
                        }
                        let click_event = UiEvent::Click { x: input.mouse_x, y: input.mouse_y };
                        if let Some(target_id) = EventSystem::hit_test(tree, input.mouse_x, input.mouse_y) {
                            event_system.0.dispatch_with_phases(&click_event, tree, target_id);
                        }
                    }
                    if input.mouse_moved {
                        let event = UiEvent::MouseMove { x: input.mouse_x, y: input.mouse_y };
                        if let Some(target_id) = EventSystem::hit_test(tree, input.mouse_x, input.mouse_y) {
                            event_system.0.dispatch_with_phases(&event, tree, target_id);
                        }
                    }
        ...
        if has_scroll {
            if let Some(event_system) = world.get_resource_mut::<EventSystemResource>() {
                if let Some(ref tree) = tree {
                    let event =
                        UiEvent::Scroll { delta_x: 0.0, delta_y: input.scroll_delta, x: input.mouse_x, y: input.mouse_y };
                    if let Some(target_id) = EventSystem::hit_test(tree, input.mouse_x, input.mouse_y) {
                        event_system.0.dispatch_with_phases(&event, tree, target_id);
                    }
                }
            ...
            if let Some(event_system) = world.get_resource_mut::<EventSystemResource>() {
                if let Some(ref tree) = tree {
                    for touch in &input.touch_events {
                        ...
                        if let Some(target_id) = EventSystem::hit_test(tree, touch.x, touch.y) {
                            event_system.0.dispatch_with_phases(&event, tree, target_id);
                        }
                    }
                }

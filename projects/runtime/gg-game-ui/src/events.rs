...
impl StandaloneInputModule {
    /// 处理点击事件
    fn handle_click(&self, x: f32, y: f32, button: MouseButton, event_system: &mut EventSystemResource, world: &mut gg_ecs::World) {
        // 射线检测，找到点击的UI元素
    ...
    /// 射线检测
    fn raycast(&self, x: f32, y: f32, world: &mut gg_ecs::World) -> Option<Entity> {
        // 实现射线检测逻辑
        // 这里只是一个占位实现
        None
    }
    ...
    fn process_click(&self, entity: Entity, event_system: &mut EventSystemResource, world: &mut gg_ecs::World) {
        // 处理点击逻辑
        // 检查是否有Button组件
        if let Some(button) = world.get_component::<crate::components::Button>(entity) {
...
impl EventSystem {
    ...
    pub fn process(&mut self, world: &mut gg_ecs::World) {
        if let Some(mut event_system) = world.get_resource_mut::<EventSystemResource>() {

use gg_ecs::World;
use gg_runtime_core::{DeltaTimer, EngineHost, FrameLimiter, FrameTime, InputEvents, ScriptEngine};
...
fn test_frame_time_fields() {
    let ft = FrameTime { delta_seconds: 0.016, fixed_delta_seconds: 0.02, elapsed_seconds: 1.0 };
...
fn test_input_events_with_keyboard() {
    ...
    let events = InputEvents { events: vec![InputEvent::Keyboard { key: KeyCode::A, state: KeyState::Pressed }] };
...
fn test_input_events_replacement() {
    ...
    world.insert_resource(InputEvents { events: vec![InputEvent::Keyboard { key: KeyCode::A, state: KeyState::Pressed }] });
    assert_eq!(world.get_resource::<InputEvents>().unwrap().events.len(), 1);
    world.insert_resource(InputEvents {
        events: vec![InputEvent::Pointer { position: (50.0, 75.0), action: PointerAction::Move, button: None }],
    });
...
fn test_frame_time_default_fixed_delta() {
    let ft = FrameTime { delta_seconds: 0.016, fixed_delta_seconds: 0.0, elapsed_seconds: 0.0 };

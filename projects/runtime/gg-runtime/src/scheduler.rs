...
impl<'a> SystemBuilder<'a> {
    ...
    /// 指定当前系统所属的并行组
    pub fn in_parallel_group(mut self, group_name: impl Into<String>) -> Self {
        if let Some(ref mut desc) = self.descriptor {
            desc.parallel_group = Some(group_name.into());
        }
        self
    }
}
...
impl<'a> Drop for SystemBuilder<'a> {
    fn drop(&mut self) {
        if let Some(descriptor) = self.descriptor.take() {
            ...
            if let Some(ref group_name) = descriptor.parallel_group {
                let group = self
                    .scheduler
                    .parallel_groups
                    .entry(group_name.clone())
                    .or_insert(ParallelSystemGroup { group_id: group_name.clone(), system_names: Vec::new() });
                group.system_names.push(descriptor.name.clone());
            }
...
pub struct StageScheduler {
    ...
    /// 并行系统组
    parallel_groups: HashMap<String, ParallelSystemGroup>,
    /// 启动阶段是否已执行
...
impl StageScheduler {
    ...
    /// 获取并行组的不可变引用
    pub fn parallel_groups(&self) -> &HashMap<String, ParallelSystemGroup> {
    ...
    fn run_stage(&mut self, stage: Stage, world: &mut World) -> GResult<()> {
        ...
        for &idx in &sorted_indices {
            if systems[idx].parallel_group.is_none() {
                serial_indices.push(idx);
            }
        }

        // Execute parallel group systems (sequential fallback until World supports concurrent access)
        for (group_name, group) in &self.parallel_groups {
            let indices: Vec<usize> = sorted_indices
                .iter()
                .filter(|&&idx| systems[idx].parallel_group.as_ref() == Some(group_name))
                .copied()
                .collect();
            ...
            for &idx in &indices {
                ...
                (system.system)(world).map_err(|e| GError {
                    ...
                    message: format!("System '{}' (parallel group '{}') failed: {}", system.name, group_name, e),
                })?;
            }
        }

        // Execute serial systems

...
pub use debug_wire::{DebugResponse, DebugWire, DebugWireError, RuntimeCommand, RuntimeState, WireMessage};
pub use diagnostic::{
    DiagnosticBuilder, DiagnosticCollector, DiagnosticLevel, DiagnosticSource, RuntimeDiagnostic, VecDiagnosticCollector,
};
pub use frame_profiler::{FrameProfile, FrameProfileReport, FrameProfiler, StageProfile};
pub use hmr::{ComponentHmrHook, HmrEvent, HmrManager, HmrMigrationResult, HmrMigrationStrategy, SerializableState};
pub use hmr_state::{ComponentStateSnapshot, StateMigrator, StateSnapshot};
...
pub use scheduler::{ParallelSystemGroup, StageScheduler};
            ...
            "print" => {
                for arg in &args {
                    match arg {
                        ...
                        BytecodeValue::List(_) => println!("[list]"),
                        BytecodeValue::Object(_) => println!("{{object}}"),
                        BytecodeValue::Map(_) => println!("{{map}}"),
                    }
                }
                None
            }
...
pub struct Runtime {
    ...
    /// 诊断收集器
    diagnostic_collector: Box<dyn DiagnosticCollector>,
    /// 帧性能分析器
...
impl Runtime {
    ...
    pub fn from_builder(builder: builder::RuntimeBuilder) -> GResult<Self> {
        ...
        Ok(Self {
            ...
            frame_limiter,
            diagnostic_collector: Box::new(VecDiagnosticCollector::new()),
            frame_profiler: if builder.enable_frame_profiler { FrameProfiler::enabled() } else { FrameProfiler::new() },
        })
    }
    ...
    /// 获取诊断收集器的不可变引用
    pub fn diagnostics(&self) -> &dyn DiagnosticCollector {
        &*self.diagnostic_collector
    }

    /// 获取诊断收集器的可变引用
    pub fn diagnostics_mut(&mut self) -> &mut dyn DiagnosticCollector {
        ...
        for plugin in self.plugin_manager.plugins() {
            if let Some(entry_fn) = plugin.bytecode.find_function(&plugin.config.entry_function) {
                ...
                match vm.execute(&plugin.bytecode, &plugin.config.entry_function, &mut self.host) {
                    VmResult::Ok | VmResult::Return(_) => {
                        let ts = self.delta_timer.elapsed().as_nanos() as u64;
                        self.diagnostic_collector.collect(
                            DiagnosticBuilder::new(
                                DiagnosticLevel::Info,
                                DiagnosticSource::Plugin,
                                format!("Plugin initialized: {}", plugin.config.name),
                            )
                            .build(ts),
                        );
                    }
                    VmResult::Error { message, .. } => {
                        ...
                        self.diagnostic_collector.collect(
                            DiagnosticBuilder::new(
                                ...
                                DiagnosticSource::Plugin,
                                format!("Plugin initialization error ({}): {}", plugin.config.name, message),
                            )
                            .build(ts),
                        );
                    }
                    ...
                    VmResult::Ok | VmResult::Return(_) => {
                        ...
                        self.diagnostic_collector.collect(
                            DiagnosticBuilder::new(
                                DiagnosticLevel::Info,
                                DiagnosticSource::Plugin,
                                format!("Plugin shutdown: {}", plugin.config.name),
                            )
                            .build(ts),
                        );
                    }
                    VmResult::Error { message, .. } => {
                        ...
                        self.diagnostic_collector.collect(
                            DiagnosticBuilder::new(
                                DiagnosticLevel::Error,
                                DiagnosticSource::Plugin,
                                format!("Plugin shutdown error ({}): {}", plugin.config.name, message),
                            )
                            .build(ts),
                        );
                    }
    ...
    pub fn tick(&mut self, delta: Duration) -> GResult<()> {
        ...
        let input_events = InputEvents { events: self.platform_services.input.poll_events() };
        self.host.world_mut().insert_resource(input_events);

        self.frame_profiler.begin_stage("HMR");
                        ...
                        VmResult::Error { message, .. } => {
                            ...
                            self.diagnostic_collector.collect(
                                DiagnosticBuilder::new(
                                    ...
                                    DiagnosticSource::Hmr,
                                    format!("HMR state migration failed: {}, rolling back", message),
                                )
                                .build(ts),
                            );
                            ...
                            Ok(()) => {
                                ...
                                self.diagnostic_collector.collect(
                                    DiagnosticBuilder::new(
                                        DiagnosticLevel::Info,
                                        DiagnosticSource::Hmr,
                                        format!("HMR: Texture reloaded: {}", asset_path),
                                    )
                                    .build(ts),
                                );
                            }
                            Err(e) => {
                                ...
                                self.diagnostic_collector.collect(
                                    DiagnosticBuilder::new(
                                        DiagnosticLevel::Error,
                                        DiagnosticSource::Hmr,
                                        format!("HMR: Failed to reload texture {}: {:?}", asset_path, e),
                                    )
                                    .build(ts),
                                );
                            }
                    ...
                    || lower.ends_with(".flac")
                {
                            ...
                            Ok(()) => {
                                ...
                                self.diagnostic_collector.collect(
                                    DiagnosticBuilder::new(
                                        DiagnosticLevel::Info,
                                        DiagnosticSource::Hmr,
                                        format!("HMR: Audio reloaded: {}", asset_path),
                                    )
                                    .build(ts),
                                );
                            }
                            Err(e) => {
                                ...
                                self.diagnostic_collector.collect(
                                    DiagnosticBuilder::new(
                                        DiagnosticLevel::Error,
                                        DiagnosticSource::Hmr,
                                        format!("HMR: Failed to reload audio {}: {:?}", asset_path, e),
                                    )
                                    .build(ts),
                                );
                            }
                    ...
                    self.diagnostic_collector.collect(
                        DiagnosticBuilder::new(
                            DiagnosticLevel::Warning,
                            DiagnosticSource::Hmr,
                            format!("HMR: Unknown asset type, skipping: {}", asset_path),
                        )
                        .build(ts),
                    );
                }
                ...
                VmResult::Error { message, .. } => {
                    ...
                    self.diagnostic_collector.collect(
                        DiagnosticBuilder::new(
                            ...
                            format!("Script update error: {}", message),
                        )
                        .build(ts),
                    );
                }
                    ...
                    VmResult::Error { message, .. } => {
                        ...
                        self.diagnostic_collector.collect(
                            DiagnosticBuilder::new(
                                DiagnosticLevel::Error,
                                DiagnosticSource::Plugin,
                                format!("Plugin update error ({}): {}", plugin.config.name, message),
                            )
                            .build(ts),
                        );
                    }
        ...
        self.frame_profiler.end_stage();
        self.frame_profiler.begin_stage("Audio");
        if let Some(ref mut audio_engine) = self.audio_engine {
        ...
        self.frame_profiler.end_stage();
        self.frame_profiler.begin_stage("Render");
        if let Some(ref mut renderer) = self.renderer {
            renderer.begin_frame()?;

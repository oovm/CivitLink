...
use gg_ui::{
    FlexDirection, FontStyle, LayoutStyle, SizeValue, Style, UiNodeData, UiNodeId, UiTree, Widget,
};
...
impl Widget for SaveSlotWidget {
    fn build(&mut self, tree: &mut UiTree) -> GResult<UiNodeId> {
        ...
        let thumbnail_style = Style::new()
            ...
            .with_layout(
                LayoutStyle::new()
                    .with_width(SizeValue::Px(120.0))
                    .with_height(SizeValue::Px(80.0)),
            );

        let thumbnail_data = if let Some(texture_id) = self.screenshot_texture_id {
            UiNodeData::Image {
                texture_id: Some(texture_id),
                size: Some((120.0, 80.0)),
            }
        } else {
            UiNodeData::Container
        };

        let thumbnail_id = tree.create_node(
            format!("SaveSlot_Thumbnail({})", self.slot),
            thumbnail_style,
            thumbnail_data,
        );
        ...
        let info_id = tree.create_node(
            format!("SaveSlot_Info({})", self.slot),
            info_style,
            UiNodeData::Container,
        );

        let slot_label_style = Style::new()
            .with_font(FontStyle::new().with_size(14.0).with_color(gg_render::Color::new(0.8, 0.8, 0.8, 1.0)));
        ...
        if self.is_empty {
            let empty_style = Style::new()
                .with_font(FontStyle::new().with_size(12.0).with_color(gg_render::Color::new(0.5, 0.5, 0.5, 1.0)));

            let empty_id = tree.create_node(
                format!("SaveSlot_Empty({})", self.slot),
                empty_style,
                UiNodeData::Text {
                    content: String::from("Empty Slot"),
                },
            );
            ...
            if let Some(ref node_id_str) = self.current_node_id {
                ...
                let chapter_id = tree.create_node(
                    ...
                    chapter_style,
                    UiNodeData::Text {
                        content: format!("Chapter: {}", node_id_str),
                    },
                );
...
impl SavePanel {
    ...
    pub fn new(slot_count: u32) -> Self {
        ...
        let slots = (0..slot_count)
            .map(|i| SaveSlotWidget::new(i).mark_empty())
            .collect();
...
impl Widget for AutoSaveIndicator {
    fn build(&mut self, tree: &mut UiTree) -> GResult<UiNodeId> {
        ...
        let text_id = tree.create_node(
            ...
            text_style,
            UiNodeData::Text {
                content: String::from("Auto Saving..."),
            },
        );

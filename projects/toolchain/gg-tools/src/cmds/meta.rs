...
fn process_directory(path: &Path) -> GResult<()> {
    for entry in fs::read_dir(path).map_err(|e| GError {
        kind: GErrorKind::Io,
...
fn generate_meta_file(file_path: &Path) -> GResult<()> {
    let stats = fs::metadata(file_path).map_err(|e| GError {
        kind: GErrorKind::Io,
    ...
    let mut meta = MetaFile::new(&asset_type, &relative_path, &name, size);
...
fn get_relative_path(file_path: &Path) -> GResult<String> {
    ...
    let relative_path = file_path
        ...
        .map_err(|e| GError { kind: GErrorKind::Runtime, message: format!("Failed to get relative path: {}", e) })?;

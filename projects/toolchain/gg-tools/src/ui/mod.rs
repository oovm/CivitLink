...
impl UiEditor for EditorUiEditor {
    fn initialize() -> GResult<Self> {
        Ok(Self { current_file: None, is_running: false })
    }
...
impl UiEditor for GameUiEditor {
    fn initialize() -> GResult<Self> {
        Ok(Self { current_scene: None, is_running: false })
    }
...
impl UiProfiler {
    /// 创建新的UI性能分析器
    pub fn new() -> Self {
        Self { performance_data: PerformanceData::default(), is_profiling: false }
    }
...
impl UiToolManager {
    /// 创建新的UI工具管理器
    pub fn new() -> GResult<Self> {
        Ok(Self { editor_ui_editor: None, game_ui_editor: None, ui_profiler: UiProfiler::new() })
    }

use gg_manifest::{
    EngineManifest, GameType,
    template::{arpg_template, custom_template, visual_novel_template},
};
#[test]
fn test_parse_valid_toml() {
    let toml_str = r#" [engine] name = "MyGame" version = "1.0.0" description = "A test game" game_type = "VisualNovel" [modules] gom = "VisualNovel" vm = "Wasm" render = "SpriteStack" plugins = ["dialogue", "portrait"] [[platforms]] target = "x86_64-pc-windows-msvc" name = "Windows" [toolchain] compiler_steps = [] editor_panels = [] [display] width = 1280 height = 720 fullscreen = false title = "MyGame" "#;

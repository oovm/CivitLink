...
fn io_error_to_gerror(err: std::io::Error) -> GError {
    GError { kind: GErrorKind::Io, message: format!("文件读取错误: {}", err) }
}

/// 创建解析错误
fn parse_error(message: &str) -> GError {
    GError { kind: GErrorKind::Runtime, message: format!("解析错误: {}", message) }
}

/// 创建编译错误
fn compile_error(message: &str) -> GError {
    GError { kind: GErrorKind::Runtime, message: format!("编译错误: {}", message) }
}
...
impl VxCompiler {
    ...
    fn parse_vx_file(&self, content: &str) -> CompilerResult<VxFile> {
        ...
        let sections = content.split("</template>").collect::<Vec<_>>();
        ...
        if sections.len() > 1 {
            ...
            let script_sections = remaining.split("</script>").collect::<Vec<_>>();
            ...
            if script_sections.len() > 1 {
                ...
                let style_part = script_sections[1].trim_start_matches("<style>").trim_end_matches("</style>");
    ...
    fn generate_code(&self, vx_file: VxFile) -> CompilerResult<String> {
        ...
        // 组合代码
        let combined_code = format!("{}\n{}\n{}", template_code, script_code, style_code);

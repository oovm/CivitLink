use gg_sheet::types::{DecimalKind, IntegerKind, SheetType, SheetValue, VectorKind};
use gg_sheet::schema::{FieldConstraint, SheetHeader, SheetTable, TableKind};
use gg_sheet::codegen::{self, CodegenConfig};
use gg_sheet::compiler::SheetCompiler;
use gg_sheet::reader::FileFormat;
...
fn test_parse_value_integer() {
    assert_eq!(SheetValue::parse_from_str("42", &SheetType::Integer(IntegerKind::Integer32)).unwrap(), SheetValue::Integer32(42));
...
fn test_parse_value_optional() {
    let result = SheetValue::parse_from_str("", &SheetType::Optional(Box::new(SheetType::Integer(IntegerKind::Integer32)))).unwrap();
    assert_eq!(result, SheetValue::Optional(None));

    let result = SheetValue::parse_from_str("42", &SheetType::Optional(Box::new(SheetType::Integer(IntegerKind::Integer32)))).unwrap();
...
fn test_generate_list_table() {
    let table = SheetTable {
        ...
        rows: vec![
            vec!["1".to_string(), "铁剑".to_string()],
            vec!["2".to_string(), "木盾".to_string()],
        ],
    };

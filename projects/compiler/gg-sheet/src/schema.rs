...
fn parse_field_name(field_str: &str) -> ParsedFieldName {
    ...
    if trimmed.starts_with("@@") {
        ParsedFieldName { name: trimmed[2..].to_string(), constraint: FieldConstraint::Primary }
    }
    else if trimmed.starts_with('@') {
        ParsedFieldName { name: trimmed[1..].to_string(), constraint: FieldConstraint::Unique }
    }
    else {
        ParsedFieldName { name: trimmed.to_string(), constraint: FieldConstraint::None }
    }
}

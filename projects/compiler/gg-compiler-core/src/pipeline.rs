...
mod tests {
    ...
    impl Transformer for RecordingTransformer {
        ...
        fn transform(
            ...
            output.insert(Artifact::new(
                ArtifactKey::new("output", &self.name),
    ...
    fn test_linear_pipeline_execution_order() {
        ...
        pipeline.add_node(
            "a",
            Box::new(RecordingTransformer::new("a", Arc::clone(&log))),
            vec![],
        );
        pipeline.add_node(
            "b",
            Box::new(RecordingTransformer::new("b", Arc::clone(&log))),
            vec!["a".to_string()],
        );
        pipeline.add_node(
            "c",
            Box::new(RecordingTransformer::new("c", Arc::clone(&log))),
            vec!["b".to_string()],
        );
    ...
    fn test_diamond_pipeline_respects_dependencies() {
        ...
        pipeline.add_node(
            "d",
            Box::new(RecordingTransformer::new("d", Arc::clone(&log))),
    ...
    fn test_incremental_compilation_skips_unchanged() {
        ...
        let initial_inputs = ArtifactSet::new();
        let result1 = pipeline.execute(initial_inputs, &mut context);
        assert!(result1.is_ok());

        let first_run_count = log.lock().unwrap().len();
        assert_eq!(first_run_count, 2);

        let same_inputs = ArtifactSet::new();
        let result2 = pipeline.execute(same_inputs, &mut context);
        assert!(result2.is_ok());

        let second_run_count = log.lock().unwrap().len();

...
use crate::{error::AotError, target::TargetPlatform};
...
impl AotBackendRegistry {
    ...
    pub fn find_backend(&self, target: &TargetPlatform) -> Option<&dyn AotBackend> {
        self.backends.iter().find(|b| b.supported_targets().contains(target)).map(|b| b.as_ref())
    }

    /// 查找支持指定目标平台的可变编译后端
    pub fn find_backend_mut(&mut self, target: &TargetPlatform) -> Option<&mut (dyn AotBackend + '_)> {
        self.backends.iter_mut().find(|b| b.supported_targets().contains(target)).map(move |b| b.as_mut())
    }
    ...
    /// 便捷方法：查找后端并编译字节码模块
    pub fn compile(&self, module: &BytecodeModule, target: &TargetPlatform) -> GResult<Vec<u8>> {
        if self.backends.is_empty() {
            return Err(GError { kind: GErrorKind::Runtime, message: AotError::NoBackendRegistered.to_string() });
        }

        match self.find_backend(target) {
            Some(backend) => backend.compile(module, target),
            None => {
                Err(GError { kind: GErrorKind::Runtime, message: AotError::UnsupportedTarget { target: *target }.to_string() })
            }
        }
    }
}

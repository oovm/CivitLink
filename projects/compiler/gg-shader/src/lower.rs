...
use naga::{
    Binding, BinaryOperator, BuiltIn, Expression, FunctionArgument, FunctionResult,
    ImageClass, ImageDimension, SampleLevel, Scalar as NagaScalar, ScalarKind, Span as NagaSpan,
    TypeInner, VectorSize,
};
use oak_valkyrie::ast::{
    Attribute, Block, Let, MicroDeclaration, Pattern, ShaderDeclaration, Statement,
    StatementNode, StructureDeclaration, TermExpression, TypeExpression,
};
use oak_valkyrie::lexer::token_type::ValkyrieTokenType;

/// 命名表达式映射类型（表达式句柄 → 名称）
type NamedExpressions = naga::FastIndexMap<naga::Handle<naga::Expression>, String>;
...
/// f32 标量常量
const F32_SCALAR: NagaScalar = NagaScalar { kind: ScalarKind::Float, width: 4 };
/// i32 标量常量
const I32_SCALAR: NagaScalar = NagaScalar { kind: ScalarKind::Sint, width: 4 };
/// u32 标量常量
const U32_SCALAR: NagaScalar = NagaScalar { kind: ScalarKind::Uint, width: 4 };
/// bool 标量常量
const BOOL_SCALAR: NagaScalar = NagaScalar { kind: ScalarKind::Bool, width: 1 };

/// 将 NamePath 转换为字符串
fn name_path_to_string(np: &oak_valkyrie::ast::NamePath) -> String {
...
impl GslLowerer {
    ...
    fn create_uniform_buffer(
        ...
        let struct_ty = module.types.insert(
            naga::Type {
                ...
                inner: TypeInner::Struct { members, span: offset },
            },
            NagaSpan::UNDEFINED,
        );
    ...
    fn lower_entry_point(
        ...
        for param in &ep.micro.params {
            ...
            let binding = self.infer_param_binding(&param.name.name, &type_name, ep.stage, &mut location_index);
        ...
        let workgroup_size = if ep.stage == naga::ShaderStage::Compute { [1u32; 3] } else { [0u32; 3] };
    ...
    /// 推断函数返回值的绑定
    fn infer_result_binding(&self, _type_name: &str, stage: naga::ShaderStage, location_index: &mut u32) -> Option<Binding> {
        if stage == naga::ShaderStage::Vertex {
    ...
    fn lower_let_statement(
        ...
        if let_stmt.is_mutable {
            ...
            let local_handle = function.local_variables.append(
                naga::LocalVariable { name: Some(var_name.clone()), ty: var_ty, init: None },
                NagaSpan::UNDEFINED,
            );
            ...
            body.push(
                naga::Statement::Store { pointer: var_expr, value: value_handle },
                NagaSpan::UNDEFINED,
            );
        } else {
    ...
    /// 创建一个 f32 零值表达式
    fn make_zero_value(module: &mut naga::Module, expressions: &mut naga::Arena<Expression>) -> GResult<naga::Handle<Expression>> {
        let ty = module.types.insert(
            naga::Type { name: None, inner: TypeInner::Scalar(F32_SCALAR) },
            NagaSpan::UNDEFINED,
        );
    ...
    fn lower_expression(
        ...
        match expr {
                ...
                if name_path.parts.len() == 1 {
                    ...
                    for part in &name_path.parts[1..] {
                        current = expressions.append(
                            Expression::AccessIndex { base: current, index: self.field_index(&part.name) as u32 },
                            NagaSpan::UNDEFINED,
                        );
                    }
                    Ok(current)
            ...
            TermExpression::DotCall { receiver, field, .. } => {
                ...
                Ok(expressions.append(Expression::AccessIndex { base, index: idx as u32 }, NagaSpan::UNDEFINED))
            }
            ...
            TermExpression::If { condition, then_branch, else_branch, .. } => {
                ...
                if let Some(else_block) = else_branch {
                    ...
                    body.push(naga::Statement::If { condition: cond, accept: then_body, reject: else_body }, NagaSpan::UNDEFINED);
                } else {
                    body.push(naga::Statement::If { condition: cond, accept: then_body, reject: naga::Block::new() }, NagaSpan::UNDEFINED);
                }
                Self::make_zero_value(module, expressions)
            }
    ...
    fn lower_call(
        ...
        match func_name.as_deref() {
            ...
            Some("vec2") | Some("vec2f") => self.lower_vec_constructor(2, args, module, function, expressions, named_expressions, body),
            Some("vec3") | Some("vec3f") => self.lower_vec_constructor(3, args, module, function, expressions, named_expressions, body),
            Some("vec4") | Some("vec4f") => self.lower_vec_constructor(4, args, module, function, expressions, named_expressions, body),
            Some("normalize") => self.lower_math_func(naga::MathFunction::Normalize, args, module, function, expressions, named_expressions, body),
            Some("length") => self.lower_math_func(naga::MathFunction::Length, args, module, function, expressions, named_expressions, body),
            Some("dot") => self.lower_math_func(naga::MathFunction::Dot, args, module, function, expressions, named_expressions, body),
            Some("mul") => {
                if args.len() == 2 {
                    ...
                    Ok(expressions.append(Expression::Binary { op: BinaryOperator::Multiply, left, right }, NagaSpan::UNDEFINED))
                } else {
                    Err(GError { kind: GErrorKind::Other, message: format!("mul() 需要 2 个参数，得到 {}", args.len()) })
                }
            }
            Some("max") => self.lower_math_func(naga::MathFunction::Max, args, module, function, expressions, named_expressions, body),
            Some("min") => self.lower_math_func(naga::MathFunction::Min, args, module, function, expressions, named_expressions, body),
            Some("clamp") => self.lower_math_func(naga::MathFunction::Clamp, args, module, function, expressions, named_expressions, body),
            Some("mix") => self.lower_math_func(naga::MathFunction::Mix, args, module, function, expressions, named_expressions, body),
            Some("pow") => self.lower_math_func(naga::MathFunction::Pow, args, module, function, expressions, named_expressions, body),
            Some("abs") => self.lower_math_func(naga::MathFunction::Abs, args, module, function, expressions, named_expressions, body),
            Some("sqrt") => self.lower_math_func(naga::MathFunction::Sqrt, args, module, function, expressions, named_expressions, body),
            Some("floor") => self.lower_math_func(naga::MathFunction::Floor, args, module, function, expressions, named_expressions, body),
            Some("ceil") => self.lower_math_func(naga::MathFunction::Ceil, args, module, function, expressions, named_expressions, body),
            Some("fract") => self.lower_math_func(naga::MathFunction::Fract, args, module, function, expressions, named_expressions, body),
            Some("step") => self.lower_math_func(naga::MathFunction::Step, args, module, function, expressions, named_expressions, body),
            Some("smoothstep") => self.lower_math_func(naga::MathFunction::SmoothStep, args, module, function, expressions, named_expressions, body),
            Some("reflect") => self.lower_math_func(naga::MathFunction::Reflect, args, module, function, expressions, named_expressions, body),
            Some("cross") => self.lower_math_func(naga::MathFunction::Cross, args, module, function, expressions, named_expressions, body),
            Some("transpose") => self.lower_math_func(naga::MathFunction::Transpose, args, module, function, expressions, named_expressions, body),
            Some("determinant") => self.lower_math_func(naga::MathFunction::Determinant, args, module, function, expressions, named_expressions, body),
            Some("inverse") => self.lower_math_func(naga::MathFunction::Inverse, args, module, function, expressions, named_expressions, body),
            Some("sign") => self.lower_math_func(naga::MathFunction::Sign, args, module, function, expressions, named_expressions, body),
            Some("sin") => self.lower_math_func(naga::MathFunction::Sin, args, module, function, expressions, named_expressions, body),
            Some("cos") => self.lower_math_func(naga::MathFunction::Cos, args, module, function, expressions, named_expressions, body),
            Some("tan") => self.lower_math_func(naga::MathFunction::Tan, args, module, function, expressions, named_expressions, body),
            Some("saturate") => self.lower_math_func(naga::MathFunction::Saturate, args, module, function, expressions, named_expressions, body),
            _ => {
    ...
    fn lower_math_func(
        ...
        if arg_handles.is_empty() {
            return Err(GError { kind: GErrorKind::Other, message: format!("数学函数 {:?} 至少需要 1 个参数", func) });
        }
    ...
    fn get_or_create_naga_type(&mut self, type_name: &str, module: &mut naga::Module) -> GResult<naga::Handle<naga::Type>> {
        ...
        let inner = match key.as_str() {
            ...
            "i32" | "int" => TypeInner::Scalar(I32_SCALAR),
            "u32" | "uint" => TypeInner::Scalar(U32_SCALAR),
            "bool" => TypeInner::Scalar(BOOL_SCALAR),
            ...
            "mat44" | "mat4x4" | "mat4x4f" => TypeInner::Matrix { columns: VectorSize::Quad, rows: VectorSize::Quad, scalar: F32_SCALAR },
            "mat33" | "mat3x3" | "mat3x3f" => TypeInner::Matrix { columns: VectorSize::Tri, rows: VectorSize::Tri, scalar: F32_SCALAR },
    ...
    fn type_expr_to_string(&self, ty: &TypeExpression) -> String {
        match ty {
            ...
            TypeExpression::Binary(bin) => format!("{} {:?} {}", self.type_expr_to_string(&bin.lhs), bin.operator, self.type_expr_to_string(&bin.rhs)),
            ...
            TypeExpression::QualifiedAssociatedType(qual) => {
                format!("<{} as {}>::{}", self.type_expr_to_string(&qual.ty), name_path_to_string(&qual.trait_path), qual.name.name)
            }
        }
    }
    ...
    fn expr_to_string(&self, expr: &TermExpression) -> String {
        match expr {
            ...
            TermExpression::Binary(node) => format!("{} {:?} {}", self.expr_to_string(&node.lhs), node.operator, self.expr_to_string(&node.rhs)),
            ...
            TermExpression::Object { callee, fields, .. } => {
                let field_strs: Vec<String> = fields.iter().map(|(name, value)| match value {

...
use gg_core::platform::{
    GamepadAxis, GamepadButton, GamepadId, Input, InputEvent, KeyCode, KeyState, PointerAction, PointerButton,
};
use gilrs::Gilrs;
...
impl DesktopInput {
    ...
    pub fn push_winit_mouse_input(&mut self, state: winit::event::ElementState, button: winit::event::MouseButton) {
        ...
        self.push_event(InputEvent::Pointer { position: self.pointer_position, action, button: Some(pointer_button) });
    }
    ...
    pub fn push_winit_cursor_moved(&mut self, position: (f32, f32)) {
        self.pointer_position = position;
        self.push_event(InputEvent::Pointer { position, action: PointerAction::Move, button: self.pointer_button });
    }
            ...
            match event {
                gilrs::EventType::ButtonPressed(button, _) => {
                    if let Some(gp_button) = Self::gilrs_button_to_gamepad(button) {
                        ...
                        events.push(InputEvent::GamepadButton { id: gamepad_id, button: gp_button, state: KeyState::Pressed });
                    }
                }
                gilrs::EventType::ButtonReleased(button, _) => {
                    if let Some(gp_button) = Self::gilrs_button_to_gamepad(button) {
                        self.gamepad_buttons.entry(gamepad_id).or_default().remove(&gp_button);
                        events.push(InputEvent::GamepadButton { id: gamepad_id, button: gp_button, state: KeyState::Released });
                    }
                }
                gilrs::EventType::AxisChanged(axis, value, _) => {
                    if let Some(gp_axis) = Self::gilrs_axis_to_gamepad(axis) {
                        ...
                        events.push(InputEvent::GamepadAxis { id: gamepad_id, axis: gp_axis, value });
                    }
                }
                gilrs::EventType::Connected => {
                    ...
                    events.push(InputEvent::GamepadConnected { id: gamepad_id, name });
                }
                gilrs::EventType::Disconnected => {
                    ...
                    events.push(InputEvent::GamepadDisconnected { id: gamepad_id });
                }
                _ => {}
            }
        }

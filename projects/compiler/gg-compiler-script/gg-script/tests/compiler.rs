...
#[test]
fn test_compile_orm_host_functions() {
    let source = r#"
        micro test_orm() {
            let result = db_query("SELECT * FROM users")
            db_insert("users", "data")
            db_update("users", 1, "data")
            db_delete("users", 1)
            let user = db_find("users", 1)
    ...
    let host_calls: Vec<&gg_ir::OpCode> =
        func.instructions.iter().filter(|op| matches!(op, gg_ir::OpCode::HostCall(_, _))).collect();
    assert!(host_calls.len() >= 6, "Expected at least 6 ORM HostCall instructions, got {}", host_calls.len());
}

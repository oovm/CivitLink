...
mod tests {
    ...
    fn test_changed_filter() {
        ...
        *world.get_component_mut::<i32>(entity).unwrap() = 100;

        let changed: Vec<Entity> = world.query_filtered::<i32, Changed<i32>>().map(|(e, _)| e).collect();
        assert_eq!(changed.len(), 1);
        assert_eq!(changed[0], entity);
    }

    #[test]
    fn test_with_without_filter() {
        let mut world = World::new();

        let e1 = world.spawn().insert(1i32).insert(10.0f64).id();
        let e2 = world.spawn().insert(2i32).id();

        let with_f64: Vec<Entity> = world.query_filtered::<i32, With<f64>>().map(|(e, _)| e).collect();
        assert_eq!(with_f64.len(), 1);
        assert_eq!(with_f64[0], e1);

        let without_f64: Vec<Entity> = world.query_filtered::<i32, Without<f64>>().map(|(e, _)| e).collect();
        assert_eq!(without_f64.len(), 1);
        assert_eq!(without_f64[0], e2);
    }
}

...
impl ListView {
    ...
    pub fn new(direction: FlexDirection, item_count: usize, item_size: [f32; 2], view_size: [f32; 2]) -> Self {
        Self {
            direction,
            item_count,
            item_size,
    ...
    fn index_at_position(&self, x: f32, y: f32) -> Option<usize> {
        ...
        if index < self.item_count { Some(index) } else { None }
    }
    ...
    fn item_style(&self, index: usize) -> Style {
        ...
        let bg_color =
            if is_selected { gg_render::Color::new(0.2, 0.4, 0.8, 0.6) } else { gg_render::Color::new(0.15, 0.15, 0.15, 0.4) };
        ...
        Style::new().with_background_color(bg_color).with_layout(LayoutStyle::new().with_width(width).with_height(height))
    }
    ...
    fn rebuild_visible_items(&mut self, ui_tree: &mut UiTree) {
        if let Some(content_id) = self.content_node_id {
            ...
            for index in first..=last {
                ...
                let item_id = ui_tree.create_node(format!("ListView_Item_{}", index), style, UiNodeData::Container);

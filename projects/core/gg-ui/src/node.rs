...
impl AccessibilityTree {
    ...
    fn build_node(tree: &UiTree, node_id: UiNodeId) -> Option<AccessibilityTreeNode> {
        ...
        let children: Vec<AccessibilityTreeNode> =
            node.children.iter().filter_map(|&child_id| Self::build_node(tree, child_id)).collect();

        let acc = &node.accessibility;
        let has_semantics = acc.role != AriaRole::None || acc.label.is_some() || acc.description.is_some() || acc.focusable;

        if has_semantics || !children.is_empty() {
            Some(AccessibilityTreeNode { node_id, accessibility: acc.clone(), children })
        }
        else {
            None
        }

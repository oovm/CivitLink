...
use crate::{
    ...
    schema::{CharacterDef, ChoiceState, DeltaTime, DialogueHistory, DialogueNode, GameVariables, HistoryEntry, WaitTimer},
};
...
impl System for DialogueSystem {
    ...
    fn execute(&mut self, world: &mut World) -> GResult<()> {
        ...
        let (speaker_id, text, commands, choices, next_node_id, node_typewriter_speed) = {
        ...
        if !text.is_empty() {
            let typewriter_speed = if node_typewriter_speed.is_some() {
                node_typewriter_speed.unwrap()
            }
            else if let Some(ref speaker) = speaker_id {
                let char_speed = world
                    .entities()
                    .iter()
                    .filter_map(|&e| world.get_component::<CharacterDef>(e))
                    .find(|c| c.id == *speaker)
                    .and_then(|c| c.typewriter_speed);
                char_speed.unwrap_or(DEFAULT_TYPEWRITER_SPEED)
            }
            else {
                DEFAULT_TYPEWRITER_SPEED
            };
            world.insert_resource(TypewriterState::new(text.clone(), typewriter_speed));
        }

        if !choices.is_empty() {
            let variables = world.get_resource::<GameVariables>();
            let filtered_choices: Vec<_> = choices
                .into_iter()
                .filter(|choice| {
                    if let Some(ref condition) = choice.condition {
                        if let Some(ref vars) = variables { ExpressionEvaluator::evaluate(condition, vars) } else { true }
                    }
                    else {
                        true
                    }
            ...
            if !filtered_choices.is_empty() {
                ...
                choice_state.choices = filtered_choices;

...
impl EditorPanel for DialogueEditorPanel {
    ...
    fn build_ui(&mut self, _context: &mut EditorContext, ui_tree: &mut UiTree) -> Option<UiNodeId> {
        let root_style =
            Style::new().with_layout(LayoutStyle::new().with_direction(FlexDirection::Column).with_padding(8.0).with_gap(4.0));
        ...
        let title_id =
            ui_tree.create_node("dialogue_title", title_style, UiNodeData::Text { content: "Dialogue Editor".to_string() });
        ui_tree.add_child(root_id, title_id);

        let list_style = Style::new().with_layout(LayoutStyle::new().with_direction(FlexDirection::Column).with_gap(2.0));
        ...
        let list_header_id =
            ui_tree.create_node("node_list_header", list_header_style, UiNodeData::Text { content: "Nodes".to_string() });
        ui_tree.add_child(list_id, list_header_id);

        if let Some(script) = &self.current_script {
            for node in &script.nodes {
                let text_summary = if node.text.len() > 30 { format!("{}...", &node.text[..30]) } else { node.text.clone() };
                ...
                let item_style =
                    Style::new().with_layout(LayoutStyle::new().with_padding(4.0)).with_font(FontStyle::new().with_size(13.0));
                let label = format!("{} [{}]: {}", node.id, speaker_label, text_summary);
                let item_id =
                    ui_tree.create_node(format!("node_item_{}", node.id), item_style, UiNodeData::Text { content: label });
        ...
        if let Some(selected_id) = &self.selected_node_id {
            ...
            let speaker_label = self.edit_state.editing_speaker_id.as_deref().unwrap_or("(none)");
            ...
            let text_id = ui_tree.create_node("prop_text", prop_style.clone(), UiNodeData::Text { content: text_preview });
            ...
            for (i, cmd) in self.edit_state.editing_commands.iter().enumerate() {
                ...
                let cmd_row_id = ui_tree.create_node(format!("prop_command_row_{}", i), cmd_row_style, UiNodeData::Container);
            ...
            let choice_list_style =
                Style::new().with_layout(LayoutStyle::new().with_direction(FlexDirection::Column).with_gap(1.0));
            let choice_list_id = ui_tree.create_node("prop_choices_list", choice_list_style, UiNodeData::Container);
            ui_tree.add_child(section_id, choice_list_id);

            for (i, choice) in self.edit_state.editing_choices.iter().enumerate() {
                let choice_row_style =
                    Style::new().with_layout(LayoutStyle::new().with_direction(FlexDirection::Row).with_gap(4.0));
                let choice_row_id =
                    ui_tree.create_node(format!("prop_choice_row_{}", i), choice_row_style, UiNodeData::Container);
                ui_tree.add_child(choice_list_id, choice_row_id);

                let choice_item_style = Style::new().with_font(FontStyle::new().with_size(12.0));
                let condition_label = choice.condition.as_deref().map(|c| format!(" [if: {}]", c)).unwrap_or_default();
            ...
            let apply_btn_id =
                ui_tree.create_node("btn_apply_edit", apply_btn_style, UiNodeData::Custom { kind: "button_apply".to_string() });
            ui_tree.add_child(section_id, apply_btn_id);
        }

        let preview_btn_style = Style::new().with_font(FontStyle::new().with_size(13.0));
        let preview_btn_kind = if self.is_previewing() { "button_stop_preview" } else { "button_start_preview" };
        let preview_btn_id =
            ui_tree.create_node("btn_preview", preview_btn_style, UiNodeData::Custom { kind: preview_btn_kind.to_string() });
        ...
        if let Some(preview) = &self.preview_state {
            ...
            let speaker_text = preview.speaker_name.as_deref().unwrap_or("Narrator");
            let speaker_id =
                ui_tree.create_node("preview_speaker", speaker_style, UiNodeData::Text { content: speaker_text.to_string() });
            ui_tree.add_child(preview_id, speaker_id);

            let text_style = Style::new().with_font(FontStyle::new().with_size(14.0));
            let text_id =
                ui_tree.create_node("preview_text", text_style, UiNodeData::Text { content: preview.displayed_text.clone() });
            ui_tree.add_child(preview_id, text_id);

            if preview.typewriter_index >= preview.full_text.len() && !preview.current_choices.is_empty() {
                let choices_style =
    ...
    fn layout_hint(&self) -> PanelLayoutHint {
        PanelLayoutHint { position: PanelPosition::Right, preferred_size: Some((350.0, 600.0)), min_size: None }
    }
}

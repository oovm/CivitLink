...
impl EditorPanel for PortraitEditorPanel {
    ...
    fn build_ui(&mut self, _context: &mut EditorContext, ui_tree: &mut UiTree) -> Option<UiNodeId> {
        ...
        let title_id =
            ui_tree.create_node("portrait_title", title_style, UiNodeData::Text { content: "Portraits".to_string() });
        ...
        for (entity_id, character_id, position) in &self.portrait_entries {
            let item_style =
                Style::new().with_layout(LayoutStyle::new().with_padding(4.0)).with_font(FontStyle::new().with_size(14.0));
            let label = format!("{} @ {}", character_id, position);
            let item_id =
                ui_tree.create_node(format!("portrait_item_{}", entity_id), item_style, UiNodeData::Text { content: label });
            ui_tree.add_child(list_id, item_id);
        }

        let button_row_style = Style::new().with_layout(LayoutStyle::new().with_direction(FlexDirection::Row).with_gap(4.0));
        ...
        let add_btn_id =
            ui_tree.create_node("btn_add_portrait", add_btn_style, UiNodeData::Custom { kind: "button_add".to_string() });

...
impl AssetCache {
    ...
    pub fn get<T: Asset>(&self, handle: &Handle<T>) -> Option<Arc<T>> {
        self.assets.get(&handle.id).and_then(|asset| {
            asset.downcast_ref::<T>().map(|t| Arc::clone(t) as Arc<T>)
        })
    }
...
pub struct AssetServer {
    cache: AssetCache,
    loaders: DashMap<TypeId, Box<dyn std::any::Any + Send + Sync>>,
}
...
impl AssetServer {
    ...
    /// 注册资源加载器
    pub fn register_loader<T: Asset, L: AssetLoader<T>>(&self, loader: L) {
        self.loaders
            .insert(TypeId::of::<T>(), Box::new(loader));
    }

    /// 异步加载资源
    pub async fn load<T: Asset, P: AsRef<Path>>(&self, path: P) -> Result<Handle<T>, AssetError> {
        let path_str = path.as_ref().to_string_lossy().to_string();
        
        if let Some(handle) = self.cache.get_handle::<T>(&path_str) {
            return Ok(handle);
        }
        ...
        let loader = self
            .loaders
            .get(&TypeId::of::<T>())
            .ok_or_else(|| AssetError::LoadError(format!("No loader registered for {}", T::type_name())))?;

        let loader = loader
            .downcast_ref::<Box<dyn AssetLoader<T>>>()
            .ok_or(AssetError::TypeMismatch)?;

        let asset = loader.load(path.as_ref()).await?;
        let handle = self.cache.insert(path_str, asset).await;

...
*.animation 文件是 GG 游戏引擎用于存储动画信息的文件格式，包含动画的关键帧、曲线、时间线等信息。动画文件是动画系统的核心组成部分，用于定义游戏对象的动画效果。

## 文件结构

一个完整的 *.animation 文件是一个 TOML 格式的文本文件，用于描述动画的属性和关键帧。
...
*.animation 文件格式为 GG 游戏引擎提供了一种统一、有效的方式来管理动画。通过存储动画的关键帧、曲线、时间线和事件，它解决了游戏对象动画效果的问题。

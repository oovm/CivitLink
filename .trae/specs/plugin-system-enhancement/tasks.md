...
- [x] Task 3: 重构对话系统命令处理器
  ...
  - [x] SubTask 3.2: 重写 `PlayBgmHandler`：从 `DialogueCommand::PlayBgm` 提取 `asset_path`、`volume`、`fade_in_secs` 写入 `AudioControl`
  - [x] SubTask 3.3: 重写 `StopBgmHandler`：从 `DialogueCommand::StopBgm` 提取 `fade_out_secs` 更新 `AudioControl`
  - [x] SubTask 3.4: 重写 `PlaySeHandler`：从 `DialogueCommand::PlaySe` 提取 `asset_path`、`volume` 添加到 `AudioControl::pending_se`
  - [x] SubTask 3.5: 重写 `ShowPortraitHandler`：从 `DialogueCommand::ShowPortrait` 提取 `character_id`、`expression`、`position` 创建 `PortraitState`
  - [x] SubTask 3.6: 重写 `HidePortraitHandler`：从 `DialogueCommand::HidePortrait` 提取 `character_id` 移除对应 `PortraitState`
  - [x] SubTask 3.7: 重写 `ChangeBackgroundHandler`：从 `DialogueCommand::ChangeBackground` 提取 `asset_path`、`transition` 更新 `SceneBackground`
  - [x] SubTask 3.8: 重写 `SetVariableHandler`：从 `DialogueCommand::SetVariable` 提取 `name`、`value` 写入 `GameVariables`
  - [x] SubTask 3.9: 重写 `WaitHandler`：从 `DialogueCommand::Wait` 提取 `duration_secs` 设置 `WaitTimer` 资源
...
- [x] Task 6: 创建 gg-plugin-tilemap 瓦片地图插件
  ...
  - [x] SubTask 6.2: 定义核心组件：`Tilemap`（地图尺寸、瓦片尺寸、图层列表）、`Tile`（全局坐标、瓦片类型、碰撞标记）
  - [x] SubTask 6.3: 定义核心资源：`Tileset`（纹理 ID、瓦片尺寸、行列数）、`TileCollisionState`（碰撞信息）
  - [x] SubTask 6.4: 实现 `TilemapRenderSystem`：遍历瓦片实体，计算纹理坐标，提交 DrawCommand
  - [x] SubTask 6.5: 实现 `TileCollisionSystem`：检查实体与碰撞瓦片重叠，更新碰撞状态
  - [x] SubTask 6.6: 实现 `TilemapPlugin`：在 `build` 中注册组件、资源和系统
  - [x] SubTask 6.7: 在根 `Cargo.toml` 中添加 `gg-plugin-tilemap` 到 workspace

- [x] Task 7: 创建 gg-plugin-spine Spine 动画插件
  - [x] SubTask 7.1: 创建 `projects/plugins/gg-plugin-spine` 目录和 `Cargo.toml`
  - [x] SubTask 7.2: 定义核心组件：`SpineSkeleton`（骨骼数据引用、当前动画、混合时间）、`SpineAnimationState`（当前轨道动画、时间、循环标记）
  - [x] SubTask 7.3: 定义核心资源：`SpineData`（骨骼定义、动画列表、附件映射）
  - [x] SubTask 7.4: 实现 `SpineAnimationSystem`：更新动画时间，计算骨骼变换
  - [x] SubTask 7.5: 实现 `SpineRenderSystem`：计算顶点位置，提交 DrawCommand
  - [x] SubTask 7.6: 实现 `SpinePlugin`：在 `build` 中注册组件、资源和系统
  - [x] SubTask 7.7: 在根 `Cargo.toml` 中添加 `gg-plugin-spine` 到 workspace
